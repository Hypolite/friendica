...
<div class="vevent event-card">
	<div class="vevent-header">
		<div class="event-card-details">
			<div class="event-card-header left-align">
				<div class="event-card-left-date left-align">
				...
				<div class="event-card-content media-body">
					...
					{{if $location.map}}<button id="event-map-btn-{{$guid}}" class="event-map-btn btn-link nav nav-pills preferences" data-map-id="event-location-map-{{$guid}}" data-show-label="{{$show_map_label}}" data-hide-label="{{$hide_map_label}}">{{$map_btn_label}}</button>{{/if}}
					<div class="event-property">
						...
						{{if $location.name}}<span class="event-location event-card-location">{{$location.name}}</span>{{/if}}
					</div>
					<div class="event-card-profile-name profile-entry-name">
						<span class="left-align1"><a href="{{$author_link}}" class="userinfo">{{$author_name}}</a></span>
					</div>
					{{if $location.map}}
					<div id="event-location-map-{{$guid}}" class="event-location-map">{{$location.map}}</div>
					{{/if}}
				</div>
