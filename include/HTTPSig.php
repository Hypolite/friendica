...
class HTTPSig
{
	...
	private static function generateDigest($body, $set = true)
	{
	...
	public static function verify($data, $key = '')
	{
		...
		if (is_array($data) && $data['header']) {
			...
			require_once 'include/HTTPHeaders.php';
...
//		}
	...
	function getActivitypubKey($id)
	{
		...
		$r = as_fetch($id); // as_fetch is not available at the moment
	...
	static function decryptSigheader($header, $prvkey = null)
	{

		$iv = $key = $alg = $data = null;

		if (!$prvkey) {
			$prvkey = Friendica\Core\Config::get('system', 'prvkey');
		}
		...
		if ($iv && $key && $alg && $data) {
			return crypto_unencapsulate(['iv' => $iv, 'key' => $key, 'alg' => $alg, 'data' => $data], $prvkey); // We need to check if we have crypto_unencapsulate
		}
		return '';
